...
* Fix: The Series REST endpoint returned nothing when asked to hide series with no sermons. The query joined the wrong table and compared a column to itself, so it failed in the database and every request came back empty. It now correctly returns series that have sermons.
