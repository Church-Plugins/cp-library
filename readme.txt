...
* New: Sermon sync can now set a sermon's service type, and supply series artwork. `SermonSync::upsert()` accepts a `thumbnail_id` on the series, used as the series featured image when the series doesn't already have one — an image chosen by hand is never overwritten.
