...
> **Important:** By default, `handle_exception()` will call `wp_send_json_error()` which terminates execution. The `$send_json` parameter defaults to `true`. If you are calling this outside of an AJAX context, pass `false` as the second parameter to avoid terminating execution:
>
> ```php
> $error_handler->handle_exception($e, false); // Logs error without terminating
> ```
