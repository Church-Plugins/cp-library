...
$has_topic_filter = !empty($active_filters['cpl_topic']);
```
...
cpl_add_filters_to_query($query, cpl_get_active_filters(), 'cpl_item');

// Manually specifying filters
$filters = [
    'cpl_topic' => ['faith', 'love'],
    'speaker' => ['john-doe']
];
...
class CustomFilterManager extends AbstractFilterManager {
    ...
    public function __construct( $args = [] ) {
        $args['post_type'] = 'your_custom_post_type';
...
```javascript
console.log('Filter query context:', window.cplFilter.query_context);
```
