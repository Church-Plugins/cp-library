...
$enabled_facets = ['cpl_topic', 'cpl_scripture', 'speaker'];
...
// Apply filters to query (modifies $query in place)
...
if (!empty($active_filters['cpl_topic'])) {
    $has_faith_filter = in_array('faith', $active_filters['cpl_topic']);
}
...
class CustomPostTypeFilterManager extends \CP_Library\Filters\AbstractFilterManager {
    ...
    /**
     * Constructor - must set post_type since FilterManager instantiates with no arguments
     */
...
function my_debug_filter_state() {
    ...
    // Get active filter manager instances
    $managers = CP_Library\Filters\FilterManager::get_active_managers();
    echo '<p>Active filter managers: ' . (empty($managers) ? 'None' : '') . '</p>';
