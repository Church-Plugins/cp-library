...
export default function ItemDescriptionEdit( {
		...
		return (
			<>
				...
				<div { ...blockProps }>
					<p>
						{ __(
							'This is the Item Description block, it will display the description for Sermons or Series.'
						) }
					</p>
				</div>
			</>
	...
	if ( isProtected && ! userCanEdit ) {
		return (
			<div { ...blockProps }>
				<Warning>
					{ __(
						'There is no description because this is a protected post.'
					) }
				</Warning>
			</div>
		);
	...
	const readMoreLink = (
		<RichText
			className="wp-block-cp-library-item-description__more-link"
			tagName="a"
	...
	const excerptClassName = classnames( 'wp-block-cp-library-item-description__excerpt', {
	...
	const excerptContent = isEditable ? (
		<RichText
			...
			aria-label={ __( 'Description text' ) }
			value={
				isSelected
					...
					: ( ! isTrimmed
					  ...
					  __( 'No description found', 'cp-library' )
			}
			onChange={ setExcerpt }
			tagName="p"
		/>
		...
		<p className={ excerptClassName }>
			{ ! isTrimmed
				? rawOrRenderedExcerpt || __( 'No description found', 'cp-library' )
				: trimmedExcerpt + ELLIPSIS }
		</p>
	);
	return (
		<>
			...
			<div { ...blockProps }>
				...
				{ ! showMoreOnNewLine && ' ' }
				{ showMoreOnNewLine ? (
					<p className="wp-block-cp-library-item-description__more-text">
						{ readMoreLink }
					</p>
				) : (
					readMoreLink
