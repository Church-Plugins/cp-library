...
class SermonFilterManager extends AbstractFilterManager {
    ...
    protected function actions() {
        ...
        add_action( 'pre_get_posts', [ $this, 'handle_service_type_query' ], 20 );
    }
    ...
    protected function register_default_facets() {
        ...
        $this->register_source_facet('speaker', 'speaker', [
            ...
            'param'             => 'facet-speaker',
            'query_var'         => 'speaker',
            'query_callback'    => [ $this, 'query_speaker_facet' ],
            'options_callback'  => [ $this, 'get_speaker_options' ],
        ]);

        // Register service type facet if enabled
        if ( Settings::get_item( 'enable_service_types', false ) ) {
            $this->register_source_facet('service-type', 'service_type', [
                'label'             => __( 'Service Type', 'cp-library' ),
                'param'             => 'facet-service-type',
                'query_var'         => 'service-type',
                'query_callback'    => [ $this, 'query_service_type_facet' ],
                'options_callback'  => [ $this, 'get_service_type_options' ],
            ]);
        }
    ...
    /**
     * Handle service type query for variations
     *
     * @param \WP_Query $query The query object
     */
    public function handle_service_type_query( $query ) {
        // Skip if not the main query or admin
        if ( is_admin() || ! $query->is_main_query() ) {
            return;
        }

        // Skip if not a query for our post type
        if ( $query->get( 'post_type' ) !== $this->post_type ) {
            return;
        }

        // Check if we have service types to filter by
        $service_types = $query->get( 'cpl_service_types' );
        if ( empty( $service_types ) ) {
            return;
        }

        // Add meta query to filter by service type
        $meta_query = $query->get( 'meta_query' ) ?: [];

        // If we have an array of service types, use IN
    ...
    /**
     * Apply speaker facet to query
     *
     * @param \WP_Query $query        The query object
     * @param array     $values       Selected speaker IDs
     * @param array     $facet_config Facet configuration
     */
    public function query_speaker_facet( $query, $values, $facet_config ) {
        if ( empty( $values ) ) {
            return;
        }
    ...
    /**
     * Apply service type facet to query
     *
     * @param \WP_Query $query        The query object
     * @param array     $values       Selected service type IDs
     * @param array     $facet_config Facet configuration
     */
    public function query_service_type_facet( $query, $values, $facet_config ) {
        if ( empty( $values ) ) {
            return;
        }
     ...
     * @return array Speaker options
     */
    public function get_speaker_options( $args = [] ) {
        global $wpdb;
        ...
        $args = wp_parse_args( $args, [
            ...
            'post_type'  => $this->post_type,
            'post__in'   => [],
            'orderby'    => Settings::get_advanced( 'sort_speaker', 'count' ),
        ]);
        ...
        $sql = $wpdb->prepare(
            ...
            INNER JOIN
                %3\$s AS type ON meta.source_type_id = type.id AND type.title = 'speaker'
            LEFT JOIN
                %4\$s AS sermon ON meta.item_id = sermon.id AND {$where_clause}
            GROUP BY
                source.id
            HAVING
                count >= %5\$d
     ...
     *
     * @param array $args Arguments
     * @return array Service type options
     */
    public function get_service_type_options( $args = [] ) {
        global $wpdb;
        ...
        $args = wp_parse_args( $args, [
            ...
            'post_type'  => $this->post_type,
            'post__in'   => [],
            'orderby'    => Settings::get_advanced( 'sort_service-type', 'count' ),
        ]);
        ...
        $sql = $wpdb->prepare(
            ...
            LEFT JOIN
                %2\$s AS meta ON meta.source_id = source.id
            INNER JOIN
                %3\$s AS type ON meta.source_type_id = type.id AND type.title = 'service_type'
            LEFT JOIN
                %4\$s AS sermon ON meta.item_id = sermon.id AND {$where_clause}
            GROUP BY
                source.id
