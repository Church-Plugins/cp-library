...
abstract class TableMeta {
	 ...
	 * @return  object
	 */
	public function get( $item_id, $key ) {
		global $wpdb;
		return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $this->table_name WHERE 'key' = %s AND 'item_id' = %d LIMIT 1;", $key, $item_id ) );
	}
	 ...
	 * @return  object
	 */
	public function get_by( $column, $value, $item_id ) {
		global $wpdb;
		$column = esc_sql( $column );
		return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $this->table_name WHERE $column = %s AND 'item_id' = %d LIMIT 1;", $value, $item_id ) );
	}
	 ...
	 * @author Tanner Moushey
	 */
	public function update_or_insert( $data, $update = true ) {
		global $wpdb;

		$data = apply_filters( 'cpl_pre_insert', $data, $this );

		if ( $update &&  $id = $this->get( $data['item_id'], $data['key'] ) ) {
